...
The report must include counts, assets placed in the package, B files awaiting cleanup, formal knowledge points, items not included, competitor/customer-case exclusions, image/non-processable counts, verification failures, changed-source failures, and remaining human decisions. Describe excluded content by reason code and locator, not by quoting or summarizing it.
